...
def generate_tests_view():
    ...
    # Save the generated tests as a CSV file
    csv_file = io.StringIO()
    csv_writer = csv.writer(csv_file)
    # Write the header
    csv_writer.writerow(['Competency', 'Question', 'Choices', 'Correct Choice'])
    # Write the test data
    csv_writer.writerows(competency_tests)

    # Rewind the StringIO object to start reading from the beginning
    csv_file.seek(0)

    # Render the result as an HTML page with the CSV data displayed
    return render_template('result.html', competency_tests=competency_tests, csv_data=csv_file.getvalue())
